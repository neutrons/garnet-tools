...
This tutorial walks through the **Peak/Shape/Refinement** and
**Calibration** tabs, which together turn a run (or set of runs) on a
well-characterized standard crystal into an updated detector geometry
and goniometer model for TOPAZ.

The physical idea
------------------
Every panel on the detector array, and the sample position itself, was
installed by hand and only *approximately* matches the idealized
geometry baked into the instrument definition. If you index Bragg
peaks using that idealized geometry, the d-spacings you compute will
disagree slightly from the true, tabulated d-spacings of the standard
- and the size and pattern of that disagreement tells you exactly how
each panel is out of place.
...
sample. The more strong peaks you have, spread across as many panels
and as wide a d-spacing range as possible, the better constrained that
geometry fit becomes.

The same run set can also pin down the goniometer: once the detector
...
and whether there's a fixed offset baked into one of the encoders.
...
- Set **Instrument** and **IPTS**, and enter the **Runs** collected on
  the standard (e.g. ``12345:12360``). Using several runs at different
  goniometer settings gives the fit angular leverage; a single run
  only constrains the panels seen at that one orientation.
- Enter the standard's **unit cell** (lengths/angles), **Crystal
  System**, **Lattice System**, and **Centering** - this is the "known
  ruler," not a guess to be refined here.
- **Max Threshold** and **Peak Radius** control how aggressively peaks
  ...
  integrated; the defaults are reasonable starting points. Behind the
  scenes, each run is converted to reciprocal space and scanned for
  tight, locally dense clusters of scattered neutrons - real Bragg
  peaks - while automatically dialing the density threshold per run so
  that a run doesn't get swamped by noise or starved by a threshold
  set for a differently-oriented run.
- Optionally supply an existing **Detector Calibration** / **Tube
  Calibration** / **UB File** if you already have a prior calibration
  or orientation to start from; otherwise the search works from the
  bare cell parameters alone.
- Click **Run**. For each run, garnet works out the crystal orientation
  ...
  common setting so peaks collected at different goniometer angles line
  up with each other.
- The output folder contains ``peaks.nxs`` (the peaks table you'll feed
  into calibration), ``peaks.mat`` (the fitted orientation), and
  ``mdhkl.nxs``, a merged reciprocal-space map you can inspect to check
  that coverage is reasonably complete and symmetric.
...
- Click **Run**. Each pass:
  ...
  4. Writes a diagnostic PDF with one plot per detector bank showing
     ...
     fit, plus an instrument-wide coverage map. A well-calibrated
     panel should show its points collapse onto the zero line; a panel
     that still trends up or down, or is offset from its neighbors,
     usually means that panel needs another look (bad peaks, wrong
     mask, or a genuinely stubborn misalignment).

- Repeat with more **Iterations** if a panel's residual hasn't
  flattened out after one pass - each iteration re-derives peak
...
- This step no longer touches panel positions. Instead, it uses the
  ...
  motor angles that were logged for each run, to fit a single rigid
  rotation-axis tilt and encoder offset that best explains every peak
  in every run at once. Physically, this captures a goniometer cradle
  that isn't perfectly plumb - a small, fixed tilt of the rotation
  axis relative to what the software assumes, plus a fixed angular
  offset on one axis (e.g. chi) between the encoder reading and the
  true zero.
...
- The fitted tilt and offset are written into the instrument
  ...
  generated the same way as in Step 2 so you can confirm the goniometer
  pass didn't reintroduce any panel residual.

Reading the output
-------------------
- ``calibration.xml`` / ``calibration.DetCal`` - the refined panel
  geometry, ready to feed into the **Peak/Shape/Refinement**,
  **Vanadium**, and main reduction tabs as the **Detector Calibration**
  input for subsequent experiments on the same cycle.
- ``calibration_goniometer.xml`` - the fitted rotation-axis tilt and
  encoder offset, present only if goniometer refinement was run.
- ``calibration_<n>.pdf`` - per-iteration diagnostics; compare the last
  one to the first to see how much the fit actually improved.
