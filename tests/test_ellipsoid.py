...
def test_missing_support_penalty_matches_manual_softplus():
    """The penalty must equal smoothing*logaddexp(0, z)/sigma with z from missing_support_fraction."""
    ellipsoid = PeakEllipsoid()
    x0, x1, x2 = _grid()
    ...
    penalty = ellipsoid.missing_support_penalty(c, inv_S, x0, x1, x2, n)

    frac, _, _ = ellipsoid.missing_support_fraction(x0, x1, x2, c, inv_S, n)
    z = (frac - ellipsoid.missing_tolerance) / ellipsoid.missing_smoothing
    expected = (
        ellipsoid.missing_smoothing
        * np.logaddexp(0.0, z)
        / ellipsoid.missing_sigma
    )

    assert np.isclose(penalty, expected)


def test_missing_support_penalty_grows_past_tolerance():
    """The penalty for a peak with no gap (fraction 0) must be much smaller than for one mostly in a gap (fraction near 1)."""
    ellipsoid = PeakEllipsoid()
    x0, x1, x2 = _grid()

    inv_S = ellipsoid.inv_S_matrix(0.5, 0.5, 0.5, 0.0, 0.0, 0.0)

    n_full = np.ones_like(x0)
    penalty_no_gap = ellipsoid.missing_support_penalty(
        (0.0, 0.0, 0.0), inv_S, x0, x1, x2, n_full
    )

    n_gap = np.ones_like(x0)
    n_gap[x0 > -0.2] = 0.0
    penalty_large_gap = ellipsoid.missing_support_penalty(
        (0.0, 0.0, 0.0), inv_S, x0, x1, x2, n_gap
    )

    assert penalty_no_gap > 0.0
    assert penalty_large_gap > penalty_no_gap * 10


def test_missing_support_penalty_jacobian_matches_outer_finite_diff():
    """The full residual/jacobian (with the missing-support term wired in) must match finite differences, given an actual gap."""
    ellipsoid = PeakEllipsoid()
    ...
    # Whether the appended missing-support row is actually nonzero here is
    # incidental to this grid's (deliberately tiny, for speed) resolution
    # relative to the peak's true width -- see
    # test_missing_support_penalty_analytic_matches_finite_diff for a
    # properly-resolved check of real, meaningfully nonzero cases. This
    # test's job is just to confirm the row is wired in consistently with
    # the residual, which the loop above already verifies.


def _penalty_c_inv_S(ellipsoid, params):
    c0 = params["c0"].value
    c1 = params["c1"].value
    c2 = params["c2"].value
    r0, r1, r2, u0, u1, u2 = ellipsoid.shape_from_params(params)
    return ellipsoid.centroid_inverse_covariance(
        c0, c1, c2, r0, r1, r2, u0, u1, u2
    )


def test_missing_support_penalty_analytic_matches_finite_diff():
    """The analytic Jacobian row must match central finite differences of missing_support_penalty, across several peak/gap geometries (plan section 21)."""
    x0, x1, x2 = _grid()
    axes = (np.eye(3)[0], np.eye(3)[1], np.eye(3)[2])
    delta = 1e-6

    geometries = [
        ("centered_no_gap", (0.5, 0.5, 0.5), (0.0, 0.0, 0.0), None),
        (
            "near_gap",
            (0.4, 0.4, 0.4),
            (1.0, 0.0, 0.0),
            lambda x0, x1, x2: x0 > 1.6,
        ),
        (
            "partly_in_gap",
            (0.5, 0.5, 0.5),
            (1.3, 0.0, 0.0),
            lambda x0, x1, x2: x0 > 1.3,
        ),
        (
            "elongated_toward_gap",
            (1.0, 0.3, 0.3),
            (0.5, 0.0, 0.0),
            lambda x0, x1, x2: x0 > 1.2,
        ),
        (
            "nearly_spherical",
            (0.45, 0.5, 0.55),
            (0.8, 0.2, -0.1),
            lambda x0, x1, x2: x0 > 1.0,
        ),
    ]

    for name, (r0, r1, r2), center, gap_fn in geometries:
        ellipsoid = PeakEllipsoid()
        _make_reference(ellipsoid, x0, x1, x2, r0, r1, r2, axes)
        ellipsoid.params["c0"].set(value=center[0])
        ellipsoid.params["c1"].set(value=center[1])
        ellipsoid.params["c2"].set(value=center[2])

        n = np.ones_like(x0)
        if gap_fn is not None:
            n[gap_fn(x0, x1, x2)] = 0.0

        c, inv_S = _penalty_c_inv_S(ellipsoid, ellipsoid.params)
        r0f, r1f, r2f, u0f, u1f, u2f = ellipsoid.shape_from_params(
            ellipsoid.params
        )
        domega = np.array(
            [
                ellipsoid.params[p].value
                for p in ("domega_x", "domega_y", "domega_z")
            ]
        )
        dr = ellipsoid.inv_S_deriv_size_shape(r0f, r1f, r2f, u0f, u1f, u2f)
        du = ellipsoid.inv_S_deriv_domega(r0f, r1f, r2f, domega)

        analytic = ellipsoid.missing_support_penalty_jacobian(
            ellipsoid.params, x0, x1, x2, n, c, inv_S, dr, du
        )

        varying = [
            pname for pname, par in ellipsoid.params.items() if par.vary
        ]
        for i, pname in enumerate(varying):
            params_p, params_m = (
                ellipsoid.params.copy(),
                ellipsoid.params.copy(),
            )
            params_p[pname].set(value=ellipsoid.params[pname].value + delta)
            params_m[pname].set(value=ellipsoid.params[pname].value - delta)

            penalty_p = ellipsoid.missing_support_penalty(
                *_penalty_c_inv_S(ellipsoid, params_p), x0, x1, x2, n
            )
            penalty_m = ellipsoid.missing_support_penalty(
                *_penalty_c_inv_S(ellipsoid, params_m), x0, x1, x2, n
            )
            fd = (penalty_p - penalty_m) / (2 * delta)

            assert np.isclose(analytic[i, 0], fd, atol=1e-6, rtol=1e-3), (
                name,
                pname,
                analytic[i, 0],
                fd,
