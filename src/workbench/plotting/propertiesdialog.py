...
#
# Vendored into garnet-tools: mantidqt's sliceviewer presenter imports
# XAxisEditor/YAxisEditor from workbench.plotting.propertiesdialog, which
# normally ships only with the full mantidworkbench conda package. We
# depend on mantidqt alone, so this shim supplies that module (and its
# sibling axiseditor.ui) without pulling in workbench.
#
...
STR_TO_COLOR_NORMALIZATION = {
    "linear": Normalize,
    "log": LogNorm,
    "symlog": SymLogNorm,
}
COLOR_NORMALIZATION_TO_STR = {
    v: k for k, v in STR_TO_COLOR_NORMALIZATION.items()
}
...
class PropertiesEditorBase(QDialog):
    ...
    def changes_accepted(self):
        raise NotImplementedError(
            "Derived classes should override changes_accepted()"
        )
...
class AxisEditor(PropertiesEditorBase):
    ...
    def create_model(self):
        ...
        memento.min, memento.max = getattr(
            self.axes, f"get_{self.axis_id}lim"
        )()
        memento.scale_mode = getattr(self.axes, f"get_{self.axis_id}scale")()
        memento.grid = (
            self.axis.grid_on()
            if hasattr(self.axis, "grid_on")
            else self.axis._major_tick_kw.get("gridOn", False)
        )
    ...
    def changes_accepted(self):
        ...
        self.limit_min, self.limit_max = float(
            self.ui.editor_min.text()
        ), float(self.ui.editor_max.text())
        self.lim_setter(self.limit_min, self.limit_max)
        self._set_tick_format()
        which = (
            "both"
            if hasattr(axes, "show_minor_gridlines")
            and axes.show_minor_gridlines
            else "major"
        )
        axes.grid(
            visible=self.ui.gridBox.isChecked(), axis=self.axis_id, which=which
        )
...
class ColorbarAxisEditor(AxisEditor):
    ...
    def changes_accepted(self):
        ...
        limit_min, limit_max = float(self.ui.editor_min.text()), float(
            self.ui.editor_max.text()
        )
    ...
    def create_model(self):
        ...
        if len(self.images) > 0:
            ...
            memento.scale_mode = COLOR_NORMALIZATION_TO_STR[
                self.images[0].norm.__class__
            ]
        else:
            memento.log = False
        memento.grid = False
...
class MarkerEditor(QWidget):
    ...
    def set_defaults(self, marker):
        ...
        _color = [
            name
            for name, symbol in self.colors.items()
            if symbol == marker.color
        ][0]
    ...
    def update_marker(self, marker):
        ...
        if new_name in self.used_names and new_name != old_name:
            raise RuntimeError(
                f"Marker names cannot be duplicated.\n Another marker is named '{new_name}'"
            )
        try:
        ...
        marker.set_color(
            self.colors.get(self.widget.color.currentText(), "C2")
        )
...
class SingleMarkerEditor(PropertiesEditorBase):
    def __init__(self, canvas, marker, valid_style, valid_colors, used_names):
        ...
        :param valid_colors: dictionary of valid colours
        """
        super(SingleMarkerEditor, self).__init__(
            "singlemarkereditor.ui", canvas
        )
        self.ui.errors.hide()

        self._widget = MarkerEditor(
            "markeredit.ui", valid_style, valid_colors, used_names
        )
...
class GlobalMarkerEditor(PropertiesEditorBase):
    def __init__(self, canvas, markers, valid_style, valid_colors):
        ...
        :param valid_colors: dictionary of valid colours
        """
        super(GlobalMarkerEditor, self).__init__(
            "globalmarkereditor.ui", canvas
        )
        self.ui.errors.hide()
        ...
        self._widget = MarkerEditor(
            "markeredit.ui", valid_style, valid_colors, self._names
        )
        layout = self.ui.layout()
